...
{
	int i, row_id;
	int rows_per_slave = size / slaves / 2;
	MPI_Status status;
...
void slave_send_result(matrix result)
{
	...
	for (i = 0; i < rows_per_slave; i++)
	{
		int row_id = myid * rows_per_slave + rows_per_slave*slaves + i;
		MPI_Send(result.element[i], size, MPI_FLOAT, MASTER_ID, row_id, MPI_COMM_WORLD);
	}
...
void master_distribute(matrix a, matrix b)
{
	...
	{	
		...
		row_start = slave_id * rows_per_slave + rows_per_slave*slaves;
		row_end = row_start + rows_per_slave;

		for (row_id = row_start; row_id < row_end; row_id++)
		{
			float row_b_buffer[size];

			for(k = 0; k < size; k++)
			{
				row_b_buffer[k] = a.element[row_id][k];
			}
			MPI_Send(row_b_buffer, size, MPI_FLOAT, slave_id, row_id, MPI_COMM_WORLD);
		}
	}
...
void master_receive_result(matrix result)
{
	...
	{	
		...
		for(i = 0; i < rows_per_slave; i++)
		{
			int row_id = slave_id * rows_per_slave + rows_per_slave * slaves + i;
			//float buffer[size];
			MPI_Recv(buffer, size, MPI_FLOAT, slave_id, row_id, MPI_COMM_WORLD, &status);
			for(j = 0; j < size; j ++)
				result.element[row_id][j] = buffer[j];
