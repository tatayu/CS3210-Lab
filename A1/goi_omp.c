...
int goi(int nThreads, int nGenerations, const int *startWorld, int nRows, int nCols, int nInvasions, const int *invasionTimes, int **invasionPlans)
{
    ...
    //!can parallel shared variables (world)?
    #pragma omp parallel for shared(world) private(row, col) 
    ...
    for (int i = 1; i <= nGenerations; i++)
    {
        ...
        if (invasionIndex < nInvasions && i == invasionTimes[invasionIndex])
        {
            ...
            //!can parallel shared variable (inv)?
            #pragma omp parallel for shared(inv) private(row, col)
            for (row = 0; row < nRows; row++)
            {
                for (col = 0; col < nCols; col++)
                {
        ...
        //!can parallel
        #pragma omp parallel for shared(wholeNewWorld, deathToll) private(row, col)
        for (row = 0; row < nRows; row++)
        {
            for (col = 0; col < nCols; col++)
            {
