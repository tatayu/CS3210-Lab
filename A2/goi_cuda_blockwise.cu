...
__global__ void updateState(int nRows, int nCols, int *world, int *inv, int *wholeNewWorld, int nTask, int dataPerThread, int remainder)
{
    //atomicAdd(&counter, 1);
    //printf("counter: %d\n", counter);
    ...
    if(tid < nTask)
    {
        ...
        //printf("tid:%d\n", tid);
        int start = dataPerThread * tid + (tid >= remainder ? remainder : tid);
        if (tid < remainder)
        {
            dataPerThread += 1;
        }

        for(int i = start; i < start + dataPerThread; i += 1)
        {
...
int goi(int gridX, int gridY, int gridZ, int blockX, int blockY, int blockZ, int nGenerations, const int *startWorld, int nRows, int nCols, int nInvasions, const int *invasionTimes, int **invasionPlans)
{
    ...
    int dataPerThread = nTask / nThread;
    int remainder = nTask % nThread;
    ...
    for (int i = 1; i <= nGenerations; i++)
    {
        ...
        updateState<<<dimGrd, dimBlk>>>(nRows, nCols, gpu_world, gpu_inv, gpu_wholeNewWorld, nTask, dataPerThread, remainder);
        check_cuda_errors();
